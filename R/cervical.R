...
# 3. DE Analysis ----
cat("\n\n", "Calculations started [", dname, "]", "\n\n\n")
  
data_path <- file.path("data")
save_path <- file.path("saved", dname)
if (!dir.exists(save_path)){
  dir.create(save_path,recursive = TRUE)
}
count <- read.csv(file.path(data_path, "count.csv"), row.names = 1)
col_data <- read.csv(file.path(data_path, "condition.csv"), row.names = 1) 

col_data <- col_data %>%
  mutate(
    condition01 = if_else(condition == dataInfo[[dname]]$positive, 1, 0),
    condition = factor(condition, levels = dataInfo[[dname]]$class_labels)
  )

if (!is.null(dataInfo[[dname]]$class_subset)){
  col_data <- col_data |> 
    dplyr::filter(condition %in% dataInfo[[dname]]$class_subset) |> 
...
nFeat <- pmin(dataInfo[[dname]]$nDEfeatures, nrow(dds_processed$DESeqObject))
...
save(results_DiffExp, file =  file.path(save_path, paste0(dname, "_results.Rda")))
cat("\n\n", "Calculations completed [", dname, "]", "\n\n\n")


# 4. Feature Select ----
load(file.path(save_path, paste0(dname, "_results.Rda")))
...
DEGs_DESeq <- tibble(Genes = geneNames) |> 
  ...
  mutate(
    ...
    DE_Status01 = case_when(
      abs(log2FoldChange) > 0.6 & log10p > -log10(0.05) ~ 1,
      .default = 0
    )
  ) |> 
...
LROC_AUC <- ggplot(plotData, aes(x = AUC, y = LROC, colour = type)) +
  geom_point(size = 2) + 
  geom_text_repel(data = repel_text_data,
    mapping = aes(label = repel_text),
    colour = colour_IG, max.overlaps = 50
  ) + 
  geom_point(data = repel_text_data, 
...
# ggsave(filename = "figure/Cervical/IG_visual_inspect_cervical.png", plot = figALL, device = png, 
#        width = 30, height = 12, units = "cm", dpi = 320, create.dir = TRUE)

# ggsave(filename = "document/manuscript/figure/IG_visual_inspect_cervical.png", plot = figALL, device = png, 
...
DESeq_Results <- DESeq_Results |> 
  mutate(
    ...
    DE_Status = factor(
      case_when(
        log2FoldChange < -0.6 & log10p > -log10(0.05) ~ -1,
        log2FoldChange > 0.6 & log10p > -log10(0.05) ~ 1,
        .default = 0
      ), 
      levels = c(-1, 0, 1),
    ...
    DE_Status2 = factor(
      case_when(
        log2FoldChange < -0.6 & pvalue < .05 ~ -1,
        log2FoldChange > 0.6 & pvalue < .05 ~ 1,
        .default = 0
      ), 
      levels = c(-1, 0, 1),
...
lapply(topIGs_not_in_DESeq_selected, function(x, .data = results_DiffExp$rocData, fig_path = file.path("figure", dname, "overall"), ...){
...
png(filename = file.path(file.path("figure", dname, "overall"), paste0("IG_genes_panel_density.png")), 
...
png(filename = file.path(file.path("figure", dname, "overall"), paste0("IG_genes_panel.png")), 
...
volcanoPlot <- ggplot(data = plotData, aes(x = log2FoldChange, y = log10p, colour = DE_Status2, label = Genes)) +
  ...
  scale_color_manual(values = c(rgb(.3, .8, .3, .2), rgb(.8, .8, .8, .2), rgb(.9, .1, .1, .2)), # to set the colours of our variable  
  ...
  guides(colour = guide_legend(override.aes = list(colour = c(rgb(.3, .8, .3), rgb(.8, .8, .8), rgb(.9, .1, .1))), 
                               position = "top", title = NULL))  + 
  geom_point(mapping = aes(x = log2FoldChange, y = -log10(pvalue)), data = repelTextData_volcanoPlot,
             color = colour_IG, size = 2) +
  geom_text_repel(mapping = aes(x = log2FoldChange, y = -log10(pvalue)), data = repelTextData_volcanoPlot,
                  color = colour_IG, max.overlaps = Inf)

print(volcanoPlot)

# ggsave(filename = "document/manuscript/figure/volcanoPlot_cervical.png", plot = volcanoPlot, device = png, 
#        width = 18, height = 14, units = "cm", dpi = 320)

# ggsave(filename = "figure/Cervical/volcanoPlot_cervical.png", plot = volcanoPlot, device = png, 
#        width = 18, height = 14, units = "cm", dpi = 320)
